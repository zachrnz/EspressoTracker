...
- (void) initialize {
    #warning Add all common code in the two initializers here
}
...
- (void)setValue:(NSDecimalNumber *)value {
    //It is important to be sure value is always an exact number, and rounded to one decimal place
    //According to some resources, using strings is the most exact way to convert a string into a DecimalNumber
    NSString *valueString = [NSString stringWithFormat:@"%f", value];
    NSDecimalNumber *valueDecimalNumber = [NSDecimalNumber decimalNumberWithString:valueString];
    NSDecimalNumber *roundedValue = [valueDecimalNumber decimalNumberByRoundingAccordingToBehavior:roundingStyle];
    _value = roundedValue;
...
- (void)setTenthsPlace:(NSInteger)tenths
{
    ...
    NSLog(@"value set to %@", self.value.stringValue);
    
    //Now, update the picker to reflect the new value
   // [self.fractionPicker selectedRowInComponent:tenths];
}
...
- (void)setOnesPlace:(NSInteger)integer
{
    ...
    self.value = newValue;
    NSLog(@"value set to %@", self.value.stringValue);
    //Update the picker to reflect the new value
    //[self.integerPicker selectedRowInComponent:integer];
}
