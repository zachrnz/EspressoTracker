 ...
 */
- (BOOL)hasInlineDatePicker //Helper method
{
 ...
 */
- (BOOL)indexPathHasPicker:(NSIndexPath *)indexPath //Helper method
{
...
/*! Determines if the given indexPath points to a cell that contains the start/end dates.
    ...
    The logic here doesn't work in cases other than the initial condition with the two
    dates in their initial positions with no datepickers displacing them. This seems to 
    be the only time the method is called, however, and so it works. The intent seems to
    be to return YES for cells containing the date, but NO for the datepicker cells 
    themselves. This is supported by comments before the only statement calling this 
    method, and also by the existing method above with the express purpose of identifing 
    the datepicker cells. I also don't understand why these values are #defined instead
    of keeping track of actual values in variables, as is done for datePickerIndexPath.
    This code seems unscalable and specific to this implementation at best, or just 
    wrong. ]
*/
- (BOOL)indexPathHasDate:(NSIndexPath *)indexPath //Helper method
{
    BOOL hasDate = NO;
    ...
    if ((indexPath.row == kDateStartRow)
        || (indexPath.row == kDateEndRow
            //I have no idea why this line of logic is applied. In cases other than
            //initial conditions, it can result in a YES being returned for the
            //datepicker cells.
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //Determine type of cell we are populating
